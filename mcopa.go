...
func parseMultipartRelated(msg io.Reader, boundary string) (textBody, htmlBody string, embeddedFiles []EmbeddedFile, err error) {
	...
	for {
		...
		case contentTypeTextPlain:
			...
			r, err := decodeContentString(textBody, part.Header.Get("Content-Transfer-Encoding"))
			if err == nil {
				textBody = r
			}
		case contentTypeTextHtml:
			...
			r, err := decodeContentString(htmlBody, part.Header.Get("Content-Transfer-Encoding"))
			if err == nil {
				htmlBody = r
			}
		case contentTypeMultipartAlternative:
...
func decodeContentString(content string, encoding string) (string, error) {
	switch encoding {
	case "base64":
